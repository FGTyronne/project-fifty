...
from project_fifty.strategies.m7 import M7_TIMEFRAME, m7_rebalance_policy
...
FIXED_DATA_END = datetime(2025, 5, 19, 23, 59, 59, tzinfo=UTC)
M6_TRADE_RATE = Decimal(101) / Decimal(424)
MAX_M7_TRADE_RATE = Decimal("0.2133")
...
def _run(
    ...
    *,
    ...
    return BacktestEngine(
        ...
        _config(),
        rebalance_policy=m7_rebalance_policy(),
    ).run(
        history=history,
...
def _gate(
    *,
    ...
    return {
        ...
        "trade_rate_within_21_33pct": trade_rate <= MAX_M7_TRADE_RATE,
        "normalized_trade_rate_lower_than_m6": trade_rate < M6_TRADE_RATE,
...
def main() -> None:
    ...
    with AlpacaMarketDataClient(data_config) as market_data:
        history = market_data.get_bars(
            ...
            timeframe=M7_TIMEFRAME,
        )

    if any(
        bar.timestamp >= datetime(2025, 5, 20, tzinfo=UTC)
        for bars in history.values()
        for bar in bars
    ):
        raise RuntimeError("M7 history overlaps the disclosed M6 dataset")
    ...
    if missing:
        raise RuntimeError(f"insufficient {M7_TIMEFRAME} history for: {','.join(missing)}")
    ...
    if split_index >= len(benchmark) - 2:
        raise RuntimeError("insufficient M7 bars for holdout split")
    ...
    payload: dict[str, object] = {
        ...
        "strategy_id": RegimePersistentConfirmedStrategy.strategy_id,
        "strategy_version": RegimePersistentConfirmedStrategy.strategy_version,
    ...
    output = Path("state/m7-economic-validation.json")
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    print("Project Fifty M7 economic validation completed")
