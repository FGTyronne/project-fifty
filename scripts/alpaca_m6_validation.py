...
from project_fifty.strategies.m6 import M6_TIMEFRAME, m6_rebalance_policy

STARTING_CASH = Decimal("67.6725")
SLIPPAGE_RATE = Decimal("0.001")
HISTORY_DAYS = 240
DEVELOPMENT_FRACTION = Decimal("0.70")
FIXED_DATA_END = datetime(2026, 1, 15, 23, 59, 59, tzinfo=UTC)
M4_TRADE_RATE = Decimal(2132) / Decimal(2499)
MAX_M6_TRADE_RATE = M4_TRADE_RATE / Decimal("4")
...
def _run(
    ...
    *,
    ...
    return BacktestEngine(
        ...
        _config(),
        rebalance_policy=m6_rebalance_policy(),
    ).run(
        history=history,
...
def _gate(
    *,
    ...
    return {
        ...
        "trade_rate_at_least_75pct_lower_than_m4": trade_rate <= MAX_M6_TRADE_RATE,
        "normalized_trade_rate_lower_than_m5": trade_rate < Decimal("0.5232558139534883721"),
...
def main() -> None:
    ...
    with AlpacaMarketDataClient(data_config) as market_data:
        history = market_data.get_bars(
            ...
            timeframe=M6_TIMEFRAME,
        )

    if any(
        bar.timestamp >= datetime(2026, 1, 16, tzinfo=UTC)
        for bars in history.values()
        for bar in bars
    ):
        raise RuntimeError("M6 history overlaps the disclosed M5 dataset")

    missing = [
        symbol
        for symbol in universe.all_symbols
        if len(history.get(symbol, ())) < 180
    ]
    if missing:
        raise RuntimeError(f"insufficient {M6_TIMEFRAME} history for: {','.join(missing)}")
    ...
    if split_index >= len(benchmark) - 2:
        raise RuntimeError("insufficient M6 bars for holdout split")
    ...
    payload: dict[str, object] = {
        ...
        "strategy_id": ConfirmedPersistentRegimeTechnicalStrategy.strategy_id,
        "strategy_version": ConfirmedPersistentRegimeTechnicalStrategy.strategy_version,
    ...
    output = Path("state/m6-economic-validation.json")
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    print("Project Fifty M6 economic validation completed")
